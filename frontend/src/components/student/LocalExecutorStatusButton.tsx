...
export function LocalExecutorStatusButton() {
  ...
  const downloadUrl = `${import.meta.env.BASE_URL}downloads/oop-local-executor-1.0.0.zip?v=20260705-cross-platform-jdk`
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-2 w-80 rounded-xl border border-slate-200 bg-white p-4 text-slate-700 shadow-xl ring-1 ring-black/5">
          ...
          <div className="mt-3 rounded-md border border-slate-200 bg-slate-50 p-3 text-xs leading-5 text-slate-600">
            Tải ZIP, giải nén rồi double-click <span className="font-bold">Start Local Executor.command</span> trên
            macOS hoặc <span className="font-bold">Start Local Executor.bat</span> trên Windows.
          </div>
