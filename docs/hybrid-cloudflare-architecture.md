...
| Node.js API hiện tại | Xác thực, phân quyền, autosave, submit, chấm điểm và audit | Giữ Render cho đến khi một compute replacement đạt các gate riêng |
| Java/Checkstyle runner | Công việc cần JVM/Linux hoặc CPU đáng kể | Không chạy trên Workers Free; chạy trên compatible compute hoặc máy sinh viên |
...
Repository hiện có foundation Cloudflare. Render là API production/rollback cho đến khi
một thay thế đạt các gate tải, bảo mật, vận hành và rollback riêng. Repository hiện có
các rào chắn sau:
